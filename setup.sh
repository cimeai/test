...
for i in \$(cat /home/3proxy/ip.list); do
    echo "$proxytype -s0 -6 -n -a -p\$port -i\$ip4_addr -e\$i"
    ((port+=1))
    ((count+=1))
...
rnd_ip_block ()
{
  ...
  g=${array[$RANDOM%16]}${array[$RANDOM%16]}${array[$RANDOM%16]}${array[$RANDOM%16]}
...
if [[ "\$mask" = "64" ]]; then
  echo \$network:\$g:\$b:\$c:\$d
elif [[ "\$mask" = "48" ]]; then
  echo \$network:\$a:\$b:\$c:\$d:\$e
elif [[  "\$mask" = "32" ]]; then
  echo \$network:\$g:\$b:\$c:\$d:\$e:\$f
else
...
touch -f /etc/update-motd.d/99-3proxy
rm -f /etc/motd
